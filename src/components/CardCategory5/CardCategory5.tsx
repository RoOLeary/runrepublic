...
const CardCategory5: FC<CardCategory5Props> = ({
  ...
  return (
    <Link
      href={href}
      className={`CardCategory5 relative block group ${className}`}
    >
      <div className="flex-shrink-0 relative w-full aspect-w-8 aspect-h-5 h-0 rounded-3xl overflow-hidden z-0 group">
        <Image
          fill
          alt="thumbnail"
          src={thumbnail || ''}
      ...
      <div className="absolute inset-0 flex items-center justify-center">
        <h2 className="text-base font-medium px-4 py-2 sm:px-6 sm:py-3 bg-white text-neutral-900 bg-opacity-50 backdrop-filter backdrop-blur-lg rounded-full border-2 border-white border-opacity-60">
          {name}
        </h2>
      </div>
    </Link>
